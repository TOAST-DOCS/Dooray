...
#### 메신저
* 데스크톱: 대화방에 초대 시, 선택 가능한 멤버로 테넌트 전체 멤버 표시
* 데스크톱: 휴면 상태에서 돌아올 때, 화면이 하얗게 되는 문제 수정
* 모바일(아이폰): 메시지 검색/모아보기 기능 추가
* 모바일(아이폰): 실행 중에 새 메시지 수신 시 알림 기능 추가
* 모바일(아이폰): 메시지에 콘텍스트 메뉴(메시지 수정,삭제) 추가
* 모바일(아이폰): 대화 목록에 콘텍스트 메뉴(스와이프) 추가
...
### 2018.08.24 
#### 메신저
* 모바일(Android): 메시지 콘텍스트 메뉴 추가
* 모바일(Android): 대화 목록 콘텍스트 메뉴(롱 탭) 추가
* 모바일(Android): 앱 실행 중 새 메시지 수신 알림 추가

### 2018.08.17
#### 메신저
* 메신저 응답 속도 개선
* 데스크톱: 윈도에서 자동실행시 설정이 초기화되는 문제 수정
* 데스크톱: 메신저 사용 중 메뉴 언어가 변경되는 오류 수정
* 데스크톱: 간헐적으로 입력이 되지 않는 문제 수정
